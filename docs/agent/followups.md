...
- [ ] FU-13 · C · src=PR#45 第九轮 APPROVE 附带 [Low] · 2026-08-06 · **`--remote` / `--remote-name` 已成摆设**:两个 flag 仍被解析、仍出现在 `--help` 的 Usage 和未知参数提示里,但 `--remote-name` 现在只喂给 `## Remote branches ($remote_name)` 这个标题,`--remote` 只开关一段说明文字。而且那段说明文字**从文档流程里已经不可达** —— status.md/run.md 都不再传 `--remote`,所以它只会对「传了一个文档已不再提及的 flag」的调用者打印。改法:把「去开 GitHub auto-delete-on-merge」这句折进默认输出或 doctor(FU-10 已记 doctor 那半),然后把两个 flag 连同 `.pilot.yml` 的 `remote:` 键一起考虑是否移除
- [ ] FU-14 · C · src=PR#45 第九轮 APPROVE 附带 [Low] · 2026-08-06 · `--integration X` 只决定**候选列表**,而真正执行的 `git branch -d` 判的是「合进 **HEAD**」。站在非 integration 分支上时,每一个真正已合并的候选都会打印 `SKIP (not safely merged)`。方向是 fail-safe(只会漏删,不会误删),但 status.md 要求模型把输出**原样**转达给用户,于是它会如实转达一个**假的「未合并」**。改法:执行前切到 integration,或改用 `git branch -d` 之外的等价判断,或在这一行的输出里说明「判据是 HEAD,不是 --integration」
