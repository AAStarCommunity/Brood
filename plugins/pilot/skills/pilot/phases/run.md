...
- **规划已经在别处的仓库**（backlog.md / issue tracker / 别的工具）：不要为了过门禁去把已有规划重抄一份到七件套里——那正违反 plan.md §A.3。在 `.pilot.yml` 里声明 `planning_requires:` 指向那个规划源的本地路径，脚本会自动读到并改查那些路径（判据不变，仍要求真有内容）。脚本输出的 `source=` 字段会写明这一轮查的是哪一种，照它汇报。
