...
**Read `templates/goal.md`，取里面的「标准版」整段**，把 `<M1/全部>`、`<集成分支>` 换成本仓库
实际值，然后原样给用户复制执行。那段不是口号，是操作契约——五个 `【】` 段分别锁死：怎么用
`docs/agent/` 的规划、怎么验证才算做完、PR 由后台 daemon 评审要怎么等、等待期间继续干什么、
什么时候才允许停。**少写哪段就会退化成哪种毛病**（见 goal.md 末尾）。

用户已经给了具体目标（「把 M1 做完」）→ 用用户的话当首句，后面四段 `【】` 照抄。
