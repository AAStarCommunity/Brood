...
4. **绝不 `git branch -D`，绝不 `git push --delete`**。删除只用 `git branch -d`（git 自己会拒绝未合并的，这是安全网），只针对「已合并 + 干净」的分支/worktree，一切经 `scripts/safe-cleanup.sh`，默认 dry-run。**squash-merge 仓库**里 `git branch --merged` 通常为空，`--squash-merged` 会按 commit 向 GitHub 核实哪个已合并 PR 引入了它，**但只把结果列出来给人，脚本不执行 `-D`**——不可逆的那一下由人来敲（详见 `reference/git-safety.md`）。
