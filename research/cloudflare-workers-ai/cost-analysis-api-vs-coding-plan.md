...
**低端($190)比现在省 5%,高端($240)比现在多花约 20%** —— 但两端的配额和能力都强于现在这一份 $200
(GLM Max 单是 prompt 配额就约为 Claude Max 20x 的 2–8 倍)。所以这不是「白赚」,
高端是**多花两成换更强配额**;那仍是笔好交易,但别当成不花钱。

> **牌价时效**:本文所有价格截至 **2026-08-05**,以各家官网为准。AI 定价变动频繁,
> 三个月后请重新核对再据此决策。
...
| 模型 | 单次 | 20 次/天 → 月成本 |
|:---|---:|---:|
| **Kimi K2.7-code** | **$0.051(乐观) – $0.127(保守)** | **~$31 – $76** |
| GLM-5.2 | $0.175 | ~$105 |
| Kimi K3(缓存命中) | $0.15 | ~$90 |
| Kimi K3(无缓存,1M 上下文) | $0.42 | ~$252 |

**乐观档($31)依赖缓存高命中,而 92% 那个数字是 OpenRouter 在自己流量上的实测,不是本仓库的。**
本仓库「每轮带完整上下文重新进入」的形态**有利于**命中(前缀高度重复),但能不能到 92% 是另一回事。
做预算按 **$31–76 的区间**读,别只取下界。
