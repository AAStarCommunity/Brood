...
> **一句话定位**：
> **Hyphae 是中小组织的"专属智能菌丝"——开源全栈（模型 + 微调 + 推理 + Agent）本地部署，token 免费；溢出能力按需调用，比 Firework 便宜。**
...
| **怎么收钱** | 训练辅导（一次性）+ 溢出 API（按用量）+ HyperCapital 运维（订阅）| **本地 token 永久免费**；用户的数据永远不离开他们的硬件 |
